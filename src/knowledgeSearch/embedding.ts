...
export function embeddingConfig(env: Record<string, string | undefined> = process.env): EmbeddingConfig {
  ...
  return { enabled: env.OKF_EMBEDDINGS_ENABLED === "true", model: model as EmbeddingConfig["model"], dimensions,
    dailyTokens, project: env.OKF_EMBEDDING_PROJECT || env.GOOGLE_CLOUD_PROJECT, location };
}
...
export function googleEmbeddings(config: EmbeddingConfig, call?: EmbedCall): EmbeddingProvider {
  ...
  return { async embed(input, kind) {
    ...
    try {
      ...
      client ??= call ? undefined : new GoogleGenAI({ vertexai: true, project: config.project, location: config.location,
