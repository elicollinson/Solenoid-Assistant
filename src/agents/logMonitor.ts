...
Review every evidence group. Use the short id returned by logs_recent (e1, e2, ...) in tool arguments and the final reviewed list. IDs are local to this scan; never copy them from earlier runs or reconstruct fingerprints. If an ID is rejected, reread logs_recent and correct it; no write occurred for that validation error. Identify actionable errors, operational issues, and recurring concerning warnings; isolated harmless warnings and routine messages need no issue. Use logs_context around evidence when useful. Use github_find_issues to find related existing incidents, including closed ones. Prefer one root-cause issue containing correlated evidence from several services over separate symptom issues. Do not reopen or duplicate a closed incident automatically.
