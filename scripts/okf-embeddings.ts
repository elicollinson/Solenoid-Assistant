...
import { googleCredentialSource } from "../src/core/googleCredentials";
...
if (command === "doctor") {
  console.log({ enabled: config.enabled, projectConfigured: !!config.project, location: config.location,
    ...
    note: "Voice API keys are not used. Credential selection does not prove Cloud IAM/model access." });
  ...
  } catch { console.error("Synthetic embedding failed. Check enabled flag, Cloud project, inline credentials or ADC, IAM, region and model access; no raw provider error is logged."); process.exitCode = 1; }
} else {
